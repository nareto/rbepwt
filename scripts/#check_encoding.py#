...
def encode_and_save(imgpath,ext,ptype,levels,wav,save=False,filepath=None,show_segmentation=False,eucl=True):
    i = rbepwt.Image()
    i.read(imgpath)
    ...
    i.encode_rbepwt(levels,wav,path_type=ptype,euclidean_distance=eucl)
...
def encode_many():
    ...
    for img in images:
        for ptype in encoding_types:
            ...
            encode_and_save(imgpath,ext,ptype,levels,wav,save=True,filepath=pickled_string,show_segmentation=False,eucl=True)
            #pickled_string='pickled/'+img+'-%s-%s-%dlevels-maxdist'%(ptype,wav,levels)
            #encode_and_save(imgpath,ext,ptype,levels,wav,save=True,filepath=pickled_string,show_segmentation=False,eucl=False)
