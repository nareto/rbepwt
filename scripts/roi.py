...
def compute_roi_coeffs(img,regionsidx):
    ...
    for level in range(1,img.rbepwt.levels+1):
        skipped_prev, prev_had_odd_len, prev_len = False, False, 0
        for regionidx, region in img.rbepwt.region_collection_at_level[level]:
            ...
            if regionidx in regionsidx:
                ...
                for i,coord in bpoints:
                    #perm = region.permutation[i]
                    #newi = int(perm/2)
                    halfi = int(i/2)
                    #perm  = underregion.permutation[halfi]
                    perm = halfi
                    coeff = (level,prev_len + perm)
                    #print("i = %d, coord = %s, perm = %d, halfi = %d, coeff = %s" %(i, coord, perm, halfi, coeff))
                    coeffset.add(coeff)
            prev_len += len(underregion)
